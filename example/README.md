...
lifecycle events, and this plugin's `regressionRun` path also executes through JUnit Platform.
...
|------|----------|---------|
| `regressionRun` | `KarateJUnit5Launcher` via `javaexec` | Karate HTML, plus JUnit-listener integrations |
| `test` + `allureReport` | Gradle `Test` + generated `KarateRunner` | Allure HTML → `build/reports/allure-report/` |

The plugin auto-generates `KarateRunner` and `KarateJUnit5Launcher` into
`build/generated-test-sources/karate-gradle/`. No runner class is needed in the consumer project.
